...
- **The stack runs an immutable `sha-<commit>` tag, never the moving `dev` one**, and the webhook
  step is what sets it — `?IMAGE_TAG=sha-…` on the POST. Portainer applies variables passed to a
  webhook to the stack, merging rather than replacing, so the secrets survive. This is not tidiness:
  a redeploy against a moving tag finds the image already present, recreates nothing, and **reports
  success while shipping nothing**. The switch that would make it re-check the registry
  (*Re-pull image*) is Business Edition only. An immutable tag needs no switch. The cost is that
  redeploying the same commit twice is a no-op, which is the correct answer to an identical commit.
