...
- **A Portainer CE webhook redeploys a Git stack only when the tracked ref has moved**, and says
  nothing when it does not: `204` in ~20ms, no pull, no recreate, and a green deploy that shipped
  nothing. *Re-pull image* — the switch that would make it act on a changed image behind an unchanged
  ref — is Business Edition. This is easy to get backwards: a deploy that lands right after a merge
  *does* replace the containers, because the git change carried the pull with it. Confirmed by
  Portainer's own `ConfigHash` moving from `b604e95f` to `1a28019c` across the one call that worked,
  and by five minutes of no change on every call after. `?IMAGE_TAG=…` does not help —
  variables-by-query belong to the *non-git* stack webhook (`"repository" !== method` in the UI).
  Deploying a *branch* never moves `main`, so the webhook alone cannot drive this pipeline; the fix is
  `PUT /api/stacks/<id>/git/redeploy` with `pullImage: true` and an API token, which is the UI's own
  "Pull and redeploy" button and is not gated. That request **replaces** the stack's environment, so
  read the stack and hand its `Env` back or `SESSION_SECRET` goes with it.
