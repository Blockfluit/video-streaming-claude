...
pipeline pushes). Leave **Re-pull image** alone; it is Business Edition only, and this pipeline is
built not to need it.

**Why it does not need it.** A redeploy is `compose up` against whatever the stack's variables say.
Point that at a moving `dev` tag and the host already holds a `dev` image, so nothing is fetched,
nothing is recreated, and Portainer reports success — a deploy that ships nothing, silently. *Re-pull
image* is the switch that would check the registry for a newer digest behind that tag, and it is
paywalled.

So the tag moves instead of the image. Every run also pushes an immutable `sha-<commit>`, and the
webhook step passes `?IMAGE_TAG=sha-<commit>` when it fires. Portainer applies variables handed to a
webhook to the stack — **merging**, so `SESSION_SECRET` and everything else survive untouched — and
redeploys. The image named is then one the host has never seen, so docker has no choice but to pull
it. No feature flag involved, and the stack's variables always name the exact commit it is running,
which the moving tag never told you.

Two consequences worth knowing:

- **`IMAGE_TAG` in the stack changes on every deploy.** Its value in
  [`stack.env.example`](stack.env.example) is only the seed for the first boot. Editing it by hand is
  how you roll back; the next deploy overwrites it again.
- **Redeploying the same commit twice pulls nothing**, because the tag is unchanged. That is correct
  — the code is identical — but if you rebuilt that commit against newer base images and want them,
  deploy a commit rather than re-running the same one.

After the first real deploy, check the container rather than the green tick:

```sh
docker inspect video-dev-api --format '{{.Config.Image}} {{.State.StartedAt}}'
```

The image should name the `sha-` tag of the commit you just deployed, with a fresh start time.
...
`docker image ls` on the server, or the GHCR package page, will list what is available. The next
deploy sets `IMAGE_TAG` again, so a rollback holds until you deliberately ship something — it is not
a pin.
