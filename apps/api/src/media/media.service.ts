...
export class MediaService implements OnModuleDestroy {
  ...
  /**
   * The same atomic dance for a banner, at hero width.
   *
   * Separate from `writeThumbnail` rather than parameterised on the key,
   * because the two differ in what they are *for* as well as where they land:
   * a thumbnail is regenerated by any probe and a banner never is, so folding
   * them together would invite a future auto-generation to write both.
   */
  private async writeBanner(videoId: string, source: string, atSeconds: number): Promise<void> {
    const key = bannerKeyFor('videos', videoId, 'jpg');
    const temporaryKey = `tmp/${videoId}-banner.jpg`;

    await this.storage.ensureDirectory('derived', 'banners/videos');
    await this.storage.ensureDirectory('derived', 'tmp');
  ...
  /** Grabs a frame to use as the wide backdrop. */
  async captureBanner(videoId: string, atSeconds: number): Promise<string> {
    const video = await this.prisma.video.findUnique({
      where: { id: videoId },
      select: { id: true, storageKey: true, playbackKey: true, bannerKey: true },
    });
    if (!video) throw new NotFoundException('No such video');

    const key = bannerKeyFor('videos', video.id, 'jpg');
    const source = this.storage.resolvePath(
      video.playbackKey ? 'derived' : 'media',
      video.playbackKey ?? video.storageKey,
    );

    await this.writeBanner(video.id, source, atSeconds);
    await this.replaceOldBanner(video.bannerKey, key);

    await this.prisma.video.update({ where: { id: video.id }, data: { bannerKey: key } });

    return key;
  }

  /** Stores an uploaded image as the wide backdrop. */
  async setBanner(videoId: string, image: Buffer, extension: string): Promise<string> {
    const video = await this.prisma.video.findUnique({
      where: { id: videoId },
    ...
    const key = bannerKeyFor('videos', video.id, extension);
  ...
  async clearBanner(videoId: string): Promise<void> {
    const video = await this.prisma.video.findUnique({
      where: { id: videoId },
      select: { id: true, bannerKey: true },
    });
    ...
    if (video.bannerKey) await this.storage.delete('derived', video.bannerKey);

    await this.prisma.video.update({ where: { id: video.id }, data: { bannerKey: null } });
  }

  /**
   * Removes the picture a banner is replacing, when it had a different name.
   *
   * The extension is part of the key, so a png replaced by a jpg does not
   * overwrite anything — it leaves the png on disk for ever, referenced by
   * nothing. `setThumbnail` has had this leak since step 10; this is the half
   * worth not copying.
   */
  private async replaceOldBanner(previousKey: string | null, newKey: string): Promise<void> {
    if (previousKey && previousKey !== newKey) {
      await this.storage.delete('derived', previousKey);
    }
  }
...
/**
 * A hero is full-bleed, so it is captured far wider than a card thumbnail.
