...
watch([q, state], () => {
  router.replace({ query: { q: q.value || undefined, state: state.value || undefined } })
})

const STATES = ['', 'DRAFT', 'PUBLISHED', 'ARCHIVED', 'MISSING']
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-3">
      ...
      <USelect v-model="state" :items="STATES" placeholder="Any state" class="w-40" />
    </div>

    <section v-if="collections?.items?.length" class="space-y-2">
      <h2 class="text-sm font-semibold tracking-wide text-white/50 uppercase">
      ...
      <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        <NuxtLink
          ...
          class="flex items-center gap-3 rounded-lg border border-white/5 bg-(--ui-bg-elevated) p-3 hover:border-white/15"
        >
          <img
          ...
          <div class="min-w-0">
            ...
            <p class="text-xs text-white/40">{{ collection.state }}</p>
          </div>
        </NuxtLink>
      </div>
    </section>
    ...
    <section class="space-y-2">
      ...
      <div v-if="videos?.items?.length" class="overflow-hidden rounded-lg border border-white/5">
        <table class="w-full text-sm">
          <tbody class="divide-y divide-white/5">
            <tr v-for="video in videos.items" :key="video.id" class="hover:bg-white/[0.03]">
              ...
              <td class="p-3 text-white/40">{{ runtime(video.durationSec) ?? '—' }}</td>
      ...
      <p v-else class="py-12 text-center text-white/40">Nothing matches.</p>
    </section>
  </div>
</template>
